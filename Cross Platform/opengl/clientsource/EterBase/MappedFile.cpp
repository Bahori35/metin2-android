...
int CMappedFile::Map(const void** dest, int offset, int size)
{
	...
	if (m_pAsset != NULL) {
		...
		if (!m_lpMapData) {
			TraceError("CMappedFile::Map AAsset_getBuffer failed");
			return 0;
		}
		
		int iViewDelta = m_dataOffset;
		m_hFM = (HANDLE)2;
    ...
    {
        TraceError("CMappedFile::Map mmap failed");
        m_lpMapData = NULL;
        return 0;
    }
...
void CMappedFile::Unmap(LPCVOID data)
{
	...
	if (m_hFM == (HANDLE)1) { // Fake handle for mmap
		long dwSysGran = sysconf(_SC_PAGE_SIZE);
		size_t dwMapViewSize = (m_dataOffset % dwSysGran) + m_mapSize;
		munmap((void*)m_lpMapData, dwMapViewSize);
		m_lpData = NULL;
		m_lpMapData = NULL;
		return;
	}
