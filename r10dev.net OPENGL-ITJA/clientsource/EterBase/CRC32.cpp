...
DWORD GetHFILECRC32(HANDLE hFile)
{
    ...
    if (fstat(fd, &st) == 0)
    {
        void* lpMapData = mmap(NULL, st.st_size, PROT_READ, MAP_PRIVATE, fd, 0);
        if (lpMapData != MAP_FAILED)
        {
            dwRetCRC32 = GetCRC32((const char*)lpMapData, st.st_size);
            munmap(lpMapData, st.st_size);
        }
    }
#endif
...
DWORD GetFileCRC32(const char* c_szFileName)
{
	...
	return dwRetCRC32;
#else
    int fd = open(c_szFileName, O_RDONLY);
    if (fd == -1) return 0;
    DWORD dwRetCRC32 = GetHFILECRC32((HANDLE)(intptr_t)fd);
    close(fd);
    return dwRetCRC32;
#endif
}
...
#else
    struct stat st;
    if (stat(c_szFileName, &st) == 0) return st.st_size;
    return 0;
#endif
}